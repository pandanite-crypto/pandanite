    ...
    if (!hosts.isDisabled()) {
        ...
        for (const auto& bestHost : topSyncedHosts) {
            try {
                ...
                Logger::logError("Failed to initialize mempool from host: " + bestHost + ". Error: " + e.what());
            }
        }
    }

    if (!mempoolInitSuccessful) {
        Logger::logError("Failed to initialize mempool after trying with top-synced hosts.");
        // You can decide further actions here.
    }
