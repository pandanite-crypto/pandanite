...
private:
  ...
  Bigint(std::vector<int> number, bool positive, int base, unsigned int skip)
      : number(std::move(number)), positive(positive), base(base), skip(skip){};

public:
  // Constructors
  Bigint();
  Bigint(long long);
  static std::optional<Bigint> from_string(const std::string &);
